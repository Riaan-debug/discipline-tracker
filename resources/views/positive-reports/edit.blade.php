    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 code-font">🌟 Edit Positive Report</h1>
                    <p class="mt-2 text-gray-600">Update student achievement details</p>
                </div>
                <a href="{{ route('positive-reports.show', $positiveReport) }}" 
        ...
        <div class="glass-effect shadow-xl rounded-2xl overflow-hidden">
            <form action="{{ route('positive-reports.update', $positiveReport) }}" method="POST" class="p-8">
                @csrf
                @method('PUT')
                        ...
                        <select name="student_id" 
                            ...
                            @foreach($students as $student)
                                <option value="{{ $student->id }}" 
                                        {{ old('student_id', $positiveReport->student_id) == $student->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="positive_report_type_id" 
                            ...
                            @foreach($positiveReportTypes as $type)
                                <option value="{{ $type->id }}" {{ old('positive_report_type_id', $positiveReport->positive_report_type_id) == $type->id ? 'selected' : '' }}>
                                    {{ $type->icon }} {{ $type->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="description" 
                              ...
                              required>{{ old('description', $positiveReport->description) }}</textarea>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="teacher_notes" 
                              ...
                              placeholder="Additional context, follow-up actions, or recognition details...">{{ old('teacher_notes', $positiveReport->teacher_notes) }}</textarea>
                    @error('teacher_notes')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
